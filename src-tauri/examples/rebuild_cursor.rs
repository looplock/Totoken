...
const CURSOR_SOURCE_APP: &str = "cursor";
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if targets.is_empty() {
        return Err("no enabled cursor scan targets configured".into());
    }
    ...
    let session_ids = load_source_session_ids(&pool, CURSOR_SOURCE_APP)?;
    println!("rebuilding {} existing Cursor sessions", session_ids.len());
    ...
    let mut summary = ScanSummary {
        ...
        root_path: "cursor targeted rebuild".to_string(),
        started_at: Utc::now(),
        files_seen: 0,
        files_parsed: 0,
        files_skipped: 0,
    ...
    let scan_result = (|| -> Result<(), Box<dyn Error>> {
        for target in targets {
            ...
            println!("scan {} {}", CURSOR_SOURCE_APP, target.path.display());
            let target_summary = scanner.scan(
                ScanRequest {
                    root_path: target.path.clone(),
                    source_app: CURSOR_SOURCE_APP.to_string(),
                    trigger_type: "manual".to_string(),
                    create_run: false,
                },
    ...
    match scan_result {
        Ok(()) => {
            ...
            println!(
                "cursor rebuild complete: rebuilt_sessions={}, rebuild_errors={}, seen={}, parsed={}, skipped={}, failed={}, sessions_changed={}, errors={}",
                rebuilt_sessions,
                rebuild_errors,
                summary.files_seen,
...
fn build_targets(settings: &SourceSettingsState) -> Vec<ScanTarget> {
    settings
        .items
        .iter()
        .filter(|item| item.enabled && item.app == CURSOR_SOURCE_APP)
